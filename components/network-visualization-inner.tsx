...
function NetworkWeb() {
  ...
  const nodes = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      temp.push({
        ...
        color: i % 2 === 0 ? "#9333ea" : "#db2777", // purple-600 and pink-600
      })
    }
    return temp
  }, [])

  // Create web-like connections between nodes
  ...
  return (
    <group ref={groupRef}>
      {nodes.map((node, i) => (
        <Float key={i} speed={1} rotationIntensity={0.2} floatIntensity={0.2} floatingRange={[-0.1, 0.1]}>
          <mesh
          ...
          {hoveredNode === i && (
            <mesh position={node.position} scale={node.scale * 1.8}>
              <sphereGeometry args={[0.15, 32, 32]} />
...
function DataParticle({
  ...
  return (
    <mesh ref={meshRef}>
      ...
      <meshBasicMaterial color={color} transparent opacity={opacity * 0.9} blending={THREE.AdditiveBlending} />
    </mesh>
  )
}
